...
mod tests {
    ...
    use crate::{models::{Balances, CreateOrderPayload, OrderSide, OrderType, SpotOrder, User}, trade::SpotOrderbook};
    ...
    fn test_create_order_and_match() {
        ...
        assert_eq!(remaining_qty, dec!(10), "Full sell order should remain unfilled");
        ...
        assert_eq!(orderbook.asks[0].quantity, dec!(5), "5 SOL should remain in ask");
        ...
        assert_eq!(user1_usdc.balance, dec!(10125), "Seller should have received 125 USDC");
        ...
        assert_eq!(user2_usdc.balance, dec!(9875), "Buyer should have spent 125 USDC");
    }
    ...
    fn test_order_matching_with_better_price() {
        ...
        // Place buy order
        let remaining_qty = orderbook.fill_orders(&buy_order, &mut users, "SOL", "USDC");
        assert_eq!(remaining_qty, dec!(0), "Buy order should be fully filled at better price");

        // Verify the trade happened at the sell order price (24 USDC)
        let users_guard = users.lock().unwrap();
        let user2 = users_guard.iter().find(|u| u.id == "2").unwrap();
        let user2_usdc = user2.balances.iter().find(|b| b.ticker == "USDC").unwrap();
        assert_eq!(user2_usdc.balance, dec!(9880), "Buyer should have spent 120 USDC (24 * 5)");
    }
    ...
    fn test_get_depth() {
        ...
        assert_eq!(
            depth.orders.get(&dec!(25)).unwrap().quantity,
            dec!(4)
        );
    }
}