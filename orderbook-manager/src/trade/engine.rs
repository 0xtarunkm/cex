...
use crate::{constants::MAX_LEVERAGE, models::{
        Balances, CreateOrderPayload, MessageFromApi, OrderSide, OrderType, Side, User 
        // MessageToApi, OpenOrdersPayload, Order,
...
*/
    ...
    fn validate_margin_requirements(&self, payload: &CreateOrderPayload) -> bool {
        ...
        let user = users.iter_mut().find(|u| u.id == payload.user_id).expect("User not found");
        ...
        let usdc_balance = user.balances.iter().find(|b| b.ticker == "USDC").map(|b| b.balance - b.locked_balance).unwrap_or(dec!(0));
